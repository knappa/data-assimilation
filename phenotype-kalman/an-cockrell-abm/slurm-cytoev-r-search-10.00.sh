#!/bin/bash
#SBATCH --job-name=cv10.00                    # Job name
...
#SBATCH --output=cv10.00-array_%A-%a.out      # Standard output and error log
...
for (( run=START_NUM; run < END_NUM; run++ )); do
  ...
  PREFIX=cytev-10.00-$(printf %04d $run)
  ...
  --uncertainty-P_DAMPS 10.0 \
  --uncertainty-T1IFN 10.0 \
  --uncertainty-TNF 10.0 \
  --uncertainty-IFNg 10.0 \
  --uncertainty-IL6 10.0 \
  --uncertainty-IL1 10.0 \
  --uncertainty-IL8 10.0 \
  --uncertainty-IL10 10.0 \
  --uncertainty-IL12 10.0 \
  --uncertainty-IL18 10.0 \
  --grid_width 51 \
  --grid_height 51 \
  --prefix $PREFIX \
