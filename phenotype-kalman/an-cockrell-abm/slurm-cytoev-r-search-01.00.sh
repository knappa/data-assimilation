#!/bin/bash
#SBATCH --job-name=cv01.00                    # Job name
...
#SBATCH --output=cv01.00-array_%A-%a.out      # Standard output and error log
...
for (( run=START_NUM; run < END_NUM; run++ )); do
  ...
  PREFIX=cytev-01.00-$(printf %04d $run)
  ...
  --uncertainty-P_DAMPS 1.0 \
  --uncertainty-T1IFN 1.0 \
  --uncertainty-TNF 1.0 \
  --uncertainty-IFNg 1.0 \
  --uncertainty-IL6 1.0 \
  --uncertainty-IL1 1.0 \
  --uncertainty-IL8 1.0 \
  --uncertainty-IL10 1.0 \
  --uncertainty-IL12 1.0 \
  --uncertainty-IL18 1.0 \
  --grid_width 51 \
  --grid_height 51 \
  --prefix $PREFIX \
