#!/bin/bash
#SBATCH --job-name=cv00.10                    # Job name
...
#SBATCH --output=cv00.10-array_%A-%a.out      # Standard output and error log
...
for (( run=START_NUM; run < END_NUM; run++ )); do
  ...
  PREFIX=cytev-00.10-$(printf %04d $run)
  ...
  --uncertainty-P_DAMPS 0.1 \
  --uncertainty-T1IFN 0.1 \
  --uncertainty-TNF 0.1 \
  --uncertainty-IFNg 0.1 \
  --uncertainty-IL6 0.1 \
  --uncertainty-IL1 0.1 \
  --uncertainty-IL8 0.1 \
  --uncertainty-IL10 0.1 \
  --uncertainty-IL12 0.1 \
  --uncertainty-IL18 0.1 \
  --grid_width 51 \
  --grid_height 51 \
  --prefix $PREFIX \
