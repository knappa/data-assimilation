#!/bin/bash
#SBATCH --job-name=wsg-ekf-sheep-grass         # Job name
...
#SBATCH --output=wsg-ekf-sheep-grass-array_%A-%a.out  # Standard output and error log
...
for (( run=START_NUM; run < END_NUM; run++ )); do
  ...
  PREFIX=sg-$(printf %04d $run)

  python3 ekf.py --prefix "$PREFIX" --measurements sheep+grass --matchmaker yes
