#!/bin/bash
#SBATCH --job-name=wsg-ekf-wolf-grass         # Job name
...
#SBATCH --output=wsg-ekf-wolf-grass-array_%A-%a.out  # Standard output and error log
...
for (( run=START_NUM; run < END_NUM; run++ )); do
  ...
  PREFIX=wg-$(printf %04d $run)

  python3 ekf.py --prefix "$PREFIX" --measurements wolves+grass --matchmaker yes
