#!/bin/bash
#SBATCH --job-name=wsg-ekf-wolf-sheep-grass         # Job name
...
#SBATCH --output=wsg-ekf-wolf-sheep-grass-array_%A-%a.out  # Standard output and error log
...
for (( run=START_NUM; run < END_NUM; run++ )); do
  ...
  PREFIX=wsg-$(printf %04d $run)

  python3 ekf.py --prefix "$PREFIX" --measurements wolves+sheep+grass --matchmaker yes
